...
def home():
    ...
    if form.validate_on_submit():
        ...
        previous_tickets = LotteryTickets.query.order_by(LotteryTickets.id.desc()).limit(10).all()

        db.session.add(LotteryTickets(ticket = "".join(str(ticket)), lottery = "".join(str(lottery)), prize = prize))
        db.session.commit()
